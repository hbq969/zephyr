...
| `config/dao/entity/ModelConfigEntity.java` | 新增 `protocol`、`maxContextTokens` 字段 |
...
| `chat/model/ChatEvent.java` | 新增 `preTokens`、`postTokens` 字段 |
...
ALTER TABLE t_model_config ADD COLUMN max_context_tokens INT DEFAULT 200000;
```

| 字段 | 默认值 | 说明 |
|------|--------|------|
| `protocol` | `openai` | `openai` 或 `anthropic` |
| `maxContextTokens` | `200000` | 模型最大上下文窗口 |
...
- **配置变更**：`protocol` 和 `maxContextTokens` 新增字段需同步更新三方言 DDL、Entity、前端表单、国际化
